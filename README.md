...
- API Key 只保存在本机：macOS Keychain 或 Windows DPAPI。
...
4. API Key；输入不会显示在终端中。

真实 Base URL、模型和 Key 都不会写入本仓库。Base URL 和模型保存在本机 `config.toml`，Key 使用系统凭据保护。
...
若安装后当前终端找不到 `codex-api`，关闭并重新打开终端再试。
...
- 不要把 `~/.codex-api`、`auth.json`、终端日志或凭据文件提交到 GitHub。
- 运行远程脚本前应先下载并检查内容。
